...
class ConcertList extends Component {
    ...
    renderConcert(concert) {
        return (
            <div className="row rounded bg-white mb-2 concert-list-item h-100" key={concert.id}>
                <div className="col-4 text-center py-2 d-none d-md-inline h-100">
                    <Link to={`/concerts/${concert.id}`}>
                        <img src="https://source.unsplash.com/random/800x1100" className="img-fluid rounded" />
                    </Link>
                </div>
                    ...
                    <div className="card h-100">
                        <div className="card-header bg-dark concert-title-header">
                            <h2 className="text-capitalize concert-title">
                                <Link className="text-white" to={`/concerts/${concert.id}`}>
                                    {concert.title}
                                </Link>
                            </h2>
                        </div>
                ...
                <div className="col-lg-3 d-none d-lg-block p-2 bg-white rounded">
                    <div className="card h-100">
                        <div className="card-header bg-primary">
                            ...
                            <ul className="list-group h-100">{_.map(concert.bands, this.renderBand)}</ul>
                    </div>
                </div>
            </div>
        );
    ...
    render() {
        ...
        return <div className="container mt-4">{_.map(orderedConcertList, this.renderConcert)}</div>;
    }
}
