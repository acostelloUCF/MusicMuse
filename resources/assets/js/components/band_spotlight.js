import React, { Component } from 'react';
import { connect } from 'react-redux';
import { fetchBand } from '../actions';
import { renderTag } from './tag_render';
import { renderLoadingSpinner } from './loading_spinner';
import Truncate from 'react-truncate-html';
import { Link } from 'react-router-dom';

class BandSpotlight extends Component {
    componentDidMount() {
        const { bandID } = this.props;
        this.props.fetchBand(bandID);
    }

    render() {
        const { band } = this.props;

        if (!band) {
            return renderLoadingSpinner();
        }

        return (
            <section id="bandShow" className="mt-4">
                <div className="container">
                    <div className="row">
                        <div className="card">
                            <div className="card-header bg-white">
                                <h3 className="text-capitalize">{band.name}</h3>
                            </div>
                            <div className="card-body">
                                <div className="row">
                                    <div className="col-md-4 band-img-col">
                                        <img
                                            src="https://source.unsplash.com/random/1200x1100"
                                            alt=""
                                            className="img-fluid rounded py-2 band-img"
                                        />
                                    </div>
                                    <div className="col-md-8 d-none d-md-block">
                                        <Truncate
                                            lines={3}
                                            dangerouslySetInnerHTML={{
                                                __html: band.description,
                                            }}
                                        />
                                    </div>
                                    <div className="col-12 text-center">
                                        <Link
                                            to={`/bands/${band.id}`}
                                            className="btn btn-primary btn-lg text-white mt-2"
                                        >
                                            Read More
                                        </Link>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        );
    }
}

function mapStateToProps({ bands }, ownProps) {
    return { band: bands[ownProps.bandID] };
}

export default connect(
    mapStateToProps,
    { fetchBand }
)(BandSpotlight);
